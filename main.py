...
def suggest_lunch_place(office_location):
    query_result = google_places.nearby_search(
        location=office_location, radius=3000, types=[types.TYPE_RESTAURANT]
    )
...
def find_weather_data(lat, lng):
    ...
    weather_time = datetime.utcfromtimestamp(
...
def fetch_geolocation(rest_address):
    restaurant_geolocation = requests.get(
        "https://maps.googleapis.com/maps/api/geocode/json?address="
        + rest_address
        + "&key=AIzaSyDmJkAbuMFs73fb44zNY21-cgytBOeUlNc"
    )
...
if __name__ == "__main__":
    ...
    with open("office.txt", encoding="utf8") as f:
        for line in f:
            ...
            # Create or update existing .csv file (file is placed in working directory)
...
# Upload .csv file to Amazon S3 (AWS) 
  # Currently commented because function requires local AWS CLI configuration to be able to upload .csv to the chosen AWS S3 bucket
